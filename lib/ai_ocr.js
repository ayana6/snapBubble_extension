...
// Function for batch processing - only OCR, no translation
...
// OCR only function for batch processing - extracts text without translation
async function recognizeViaVisionOCROnly(dataUrl, aiModel, settings, signal) {
  // Determine provider from aiModel
...
// Batch AI Vision processing for multiple images
export async function batchRecognizeViaVision(images, opts = {}) {
  const settings = await loadSettings();
  const results = [];
  
  // Check if we should stop processing due to repeated AI errors
  ...
  // Process images in batches of 2 for AI Vision (more expensive)
  const batchSize = 2;
  for (let i = 0; i < images.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (img) => {
      try {
        const dataUrl = await imgElToJpegDataUrl(img);
        const result = await recognizeViaVision(dataUrl, opts);
        return result || { words: [], text: "" };
      } catch (e) {
        // Track AI Vision errors
        await trackError("ai_vision_failed", `Batch AI Vision failed: ${e?.message || e}`, settings.aiModel);
        const hudMsg = `[ERR] Batch AI Vision failed: ${e?.message || e}`;
        try {
          if (typeof window !== 'undefined' && window.logHud) {
    ...
    // Longer delay between AI Vision batches to be respectful to the API
