...
export async function loadSettings() {
  ...
  const localData = await chrome.storage.local.get(['personalOcrKey', 'personalTranslateKey', 'personalTranslateKeys']);
  ...
  if (Array.isArray(localData.personalTranslateKeys) && localData.personalTranslateKeys.length) {
    merged.translateApiKeys = localData.personalTranslateKeys.filter(k => typeof k === 'string' && k.trim()).map(k => k.trim());
    if ((!merged.translateApiKey || !merged.translateApiKey.trim()) && merged.translateApiKeys.length) {
      merged.translateApiKey = merged.translateApiKeys[0];
    }
  } else if (localData.personalTranslateKey) {
  ...
  if (!Array.isArray(merged.translateApiKeys)) merged.translateApiKeys = [];
