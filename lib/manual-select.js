...
async function showResultOverlay(rect, result, settings) {
  ...
  if (result.words && result.translated) {
    ...
    for (let i = 0; i < Math.min(result.words.length, result.translated.length); i++) {
      ...
      const t = (result.translated[i] || '').trim();
      if (!t) continue;
      items.push({
        left: w.left * scaleX,
        top: w.top * scaleY,
        width: Math.max(w.width * scaleX, 8),
        height: Math.max(w.height * scaleY, 8),
        text: t
      });
    }
    ...
    const t = (typeof result === 'string') ? result : (result.text || '');
    if (t.trim()) {
      items.push({ left: rect.width * 0.05, top: rect.height * 0.05, width: rect.width * 0.9, height: rect.height * 0.9, text: t.trim() });
    }
  }

  const { groupWordRects } = await import(chrome.runtime.getURL('lib/segmentation.js'));
  const boxes = (items.length > 0) ? groupWordRects(items).boxes : [];
